...
Customer::Customer(int type_){
    type = type_;
    rentalCount = 0;
    id = Customer.createId();
    if(type == TYPE_PERSON){
        maxCount = RENTAL_MAX_PERSON;
    }
    else{
        maxCount = RENTAL_MAX_COMPANY;
    }
    rentalList = new int[maxCount];
    for(int i = 0; i < maxCount; i++){
        rentalList[i] = 0;
    }
}

Customer::Customer(const Customer &r){
    type = r.type;
    rentalCount = r.rentalCount;
    maxCount = r.maxCount;
    for(int i = 0; i < maxCount; i++){
        rentalList[i] = r.rentalList[i];
    }
    id = r.id;
}
...
Customer& Customer::operator=(const Customer &r){
    Customer newCustomer(r);
    return newCustomer;
}

bool Customer::addRental(int productId){
    if(rentalCount == maxCount) return false;
    rentalList[rentalCount] = productId;
    rentalCount++;
    return true;
}

int Customer::findRentalIndex(int productId) const{
    for(int i = 0; i < rentalCount; i++){
        if(rentalList[i] == productId){
            return i;
        }
    }
    return NOT_USED;
...
bool Customer::removeRental(int productId){
    int index = findRentalIndex(productId);

    if(findRentalIndex(productId) == NOT_USED) return false;

    for(int i = index; i < maxCount - 1; i++){
        rentalList[i] = rentalList[i+1];
    }
    rentalList[maxCount - 1] = 0;
    rentalCount--;
    return true;
}

// 고객id, 대여 제품 수, 최대 대여 가능 제품 수 출력
void Customer::showInfo() const{
    cout<<"["<<id<<", "<<rentalCount<<"/"<<maxCount<<"] ";
}

static int Customer::gCount = 0;

int Customer::createId(){
    int res = Customer.gCount;
    Customer.gCount++;
    return res;
}
